...
function initializeChartsCombobox() {
	...
	var compoboxHTML = '<select id="mCP" onchange="izomorphismChartInputChange(this)" class="select-drop-down graph-select">';
...
function initializeFromToNodesCombobox() {
	...
	// from node
	var compoboxHTML = '<select id="fromNodeCB" onchange="fromNodeSPInputChange(this)" class="select-drop-down graph-nodes">';
	...
	// to node
	var compoboxHTML = '<select id="toNodeCB" onchange="toNodeSPInputChange(this)" class="select-drop-down graph-nodes">';
	for (var i = 0; i < listOfNodes.length; i++) {
		compoboxHTML += '<option value="' + listOfNodes[i].id + '">'
				+ listOfNodes[i].name + '</option>';
	}
...
function createNewGraph(graphOptions) {
	...
	if (NUM_OF_GRAPHS < 20) {
		...
		alert("Maximum number of open graphs is 20, please close to continue");
	}
}
/**
...
function saveGraph() {
	...
	var filename = (MAIN_GRAPHS[graphId].graphTitle + ".xml");
	var pom = document.createElement('a');
